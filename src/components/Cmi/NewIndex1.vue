  ...
  <div class="index">
    ...
    <div class="bbc-index">
      <div class="qiandao">
        ...
        <div class="xxk">
          <ul ref="tit">
            <li v-for="(item, index) in dateList" :key="item.day" :class="item.active" @click="handleToShowPoster(index)"><span>{{item.active == 'active ago' ? '' : item.date}}</span><p>{{item.cmi}}<i></i></p></li>
          </ul>
      ...
      <div style="position: absolute; left: 50%; top: 50%; transform: translate(-50%,-50%);">
        <div style="position: relative; width: 6.4rem; height: 10.4rem; margin: auto;">
          <div class="haibao_haibaoImg photo-box">
            <div class="bgImg">
              <img :src="HaiBaoMsg.picUrl" alt="" class="haibao_ImgUrl" crossOrigin="anonymous">
              <div class="bgCon">
                <div class="headerImg">
                  <img v-if="HaiBaoMsg.headUrl" :src="'http://testuser.meibbc.com/oss'+HaiBaoMsg.headUrl" alt="" crossOrigin="anonymous">
                  <img v-if="!HaiBaoMsg.headUrl && HaiBaoMsg.sex == 2" :src="require('../../assets/img/newIndex/header_nv.png')" alt="" crossOrigin="anonymous">
                  <img v-if="!HaiBaoMsg.headUrl && HaiBaoMsg.sex == 1" :src="require('../../assets/img/newIndex/header_nan.png')" alt="" crossOrigin="anonymous">
                  <img v-if="!HaiBaoMsg.headUrl && HaiBaoMsg.sex == null" :src="require('../../assets/img/newIndex/header_nan.png')" alt="" crossOrigin="anonymous">
                </div>
                <div class="bgtime">{{nowTime}}</div>
                <div class="bgweather" v-show="HaiBaoMsg.cityName">{{HaiBaoMsg.cityName}} {{HaiBaoMsg.temperature}}°C{{HaiBaoMsg.text}}</div>
                <div class="bgweather" v-show="!HaiBaoMsg.cityName">暂时无法定位</div>
                <div class="bgday">
                  <p>{{new Date().getDate()}}</p>
                  {{new Date().getMonth() | changeMonthToEnglish}} {{new Date().getFullYear()}}
                </div>
                <div class="bgwisdom">{{HaiBaoMsg.mottoContent}}<br>—— {{HaiBaoMsg.writer}}</div>
                <div class="bgnumber">
                  <p><span>{{HaiBaoMsg.signDay}}</span>累计签到(天) <img :src="require('../../assets/img/newIndex/bgborder.png')" alt="" crossOrigin="anonymous"></p>
                  <p><span>{{HaiBaoMsg.joinCount}}</span>参与用户 <img :src="require('../../assets/img/newIndex/bgborder.png')" alt="" crossOrigin="anonymous"></p>
                  <p><span>{{HaiBaoMsg.percentage ? HaiBaoMsg.percentage : 0 }}</span>超越用户</p>
                  <!-- <p><span>{{HaiBaoMsg.percentage ? HaiBaoMsg.percentage : 0 }}</span>超越用户（签到时间）</p> -->
                </div>
              </div>
            </div>
            <div class="bgfooter">
              <div class="bgtip">
                <p>美胸汇，让你更美</p>
                <span>{{HaiBaoMsg.copyWriting}}</span>
              </div>
              <div class="bgewm">
                <!-- <p><img :src="require('../../assets/img/app_erweima1.png')" alt="" crossOrigin="anonymous"></p> -->
                <p><img :src="require('../../assets/img/ceshidown.png')" alt="" crossOrigin="anonymous"></p>
                <span>美胸汇App</span>
              </div>
            </div>
          </div>
          <!-- <div v-if="trueImgUrl" class="haibao_haibaoImg">
            <img :src="trueImgUrl" alt="" style="width: 100%; height: 100%;">
          </div> -->
          <div class="haibao_close" @click="isHaiBaoShow = false"></div>
          <div class="bgloading" v-show="showHaiBaoLoading">
            <van-loading size="24px" vertical color="#F63B75">海报生成中...</van-loading>
          </div>
        </div>
...
import { systemt_wxFun } from '@/assets/js/wxShare'
...
export default {
  ...
  methods: {
    getHaiBaoMsg(callback) {
      let params = {
        userId: this.userId,
        cityName: this.city
      }
      USER_API.getHaiBaoMsg(params).then(res => {
        console.log(res)
        if (res) {
          this.HaiBaoMsg = res;
      ...
      if(this.isCheck){
        ...
        USER_API.addsignlog(params).then(data=>{
          ...
          if(data){
            ...
            this.nowTime = this.returnDate(new Date());
            localStorage.setItem('signTime', this.nowTime)
            this.loadIsSigned();
            this.loadUser();
            this.getHaiBaoMsg();
    ...
    isTodayTrue(max,maxNumber,min,minNumber,cmiNumber,active,time){
      let dateList = [];
      let count = Math.floor(this.totalNumber / 7)
      let dataTime = [];
      for (var i = 1; i <= 7; i++) {
        dataTime.push((count * 7 + i) + ' Day')
      }
    ...
    returnCmi(cmiData){
      let cmi = 10;
      if (Math.floor(this.totalNumber / 7) == 0) {
    ...
    shareFun(type){ 
      ...
      systemt_wxFun(
        '恭喜您获得美胸汇平台入驻邀请',
        '黄金时机入驻，轻松引流拓客，点击查看',
        'https://oss.meibbc.com/BeautifyBreast/logo/logo.png',
        'http://testuser.meibbc.com/bbc-personal/sharerImg?url=' + this.trueImgUrl,
        1
      )
    },
    getHaiBaoBackground() {
    ...
    createPhoto() {
      ...
      if (this.isHaiBaoShow) {
        ...
        html2canvas(photoBox, opts).then(function(canvas) {
          ...
          let bold = that.dataURLtoBlob(dataUrl);
          let fileObj = that.blobToFile(bold, "123.png");
          ...
          if (fileObj) {
            ...
            that.$http({
              ...
              if (that.trueImgUrl) {
                ...
                localStorage.setItem('trueImgUrl', that.trueImgUrl)
          ...
          if (that.shareImgUrl) {
            ...
            localStorage.setItem('shareImgUrl', that.shareImgUrl)
          }
        });
      }
      }catch(e){
    ...
    dataURLtoFile(dataurl, filename) {
      ...
      //   class File {
      //     constructor(){
      //       bold = that.dataURLtoBlob(base);
      //       fileObj = that.blobToFile(bold, "123.png");
      //       return fileObj
      //     }
      //   }
      // }
    ...
    // 点击当天显示海报
    handleToShowPoster(index) {
      if (this.isSign) {
        if (this.isSignNumber % 7 == index + 1 || this.isSignNumber % 7 == 0) {
          this.getHaiBaoMsg()
          this.nowTime = localStorage.getItem('signTime')
          if (localStorage.getItem('trueImgUrl') && localStorage.getItem('shareImgUrl')) {
            this.trueImgUrl = localStorage.getItem('trueImgUrl');
            this.shareImgUrl = localStorage.getItem('shareImgUrl');
...
.haibao_haibaoImg {min-width:100px; width: 6.4rem; min-height: 100px; height: 10.04rem; border: .01rem solid #000; margin: auto; /*margin-top: 1.28rem;*/ margin-bottom: .4rem; position: relative; background: #fff;}
...
.bgCon {width: 5.94rem; height: 2.94rem; background: rgba(0,0,0,.65); border-radius: .2rem .2rem 0 0; position: absolute; bottom: 0.0005rem; left: 50%; margin-left: -2.97rem; padding: .28rem .4rem 0 .4rem;}
.headerImg {width: 1.48rem; height: 1.48rem; border-radius: 50%; position: absolute; top: -.74rem; right: 0; left: 0; margin: auto;}
...
.bgweather {font-size: .22rem; color: #48CBD7; margin-bottom: .08rem;}
...
.bgnumber p {flex: 1; font-size: .2rem; color: #FFF4F4; text-align: center; position: relative;}
.bgnumber p img {position: absolute; width: .01rem; height: .84rem; right: 0; top: 50%; margin-top: -.42rem;}
/* .bgnumber p:nth-child(3) {flex: 1.5;} */
/* .bgnumber p:nth-child(1)::after {content: ""; display: block; width: .02rem; height: .84rem; position: absolute; right: 0; top: 50%; transform: translateY(-50%);} */
...
.bgtip p {font-size: .24rem; color: #F63B75; margin-bottom: .1rem;}
.bgtip span {display: block; width: 83%;}
/* .bgtip span {display: block; transform: scale(.8) translateX(-.75rem);} */
.bgewm {position: absolute; width: .93rem; right: .2rem; top: 0;}
.bgewm p {width: .93rem; height: .93rem; text-align: center;}
.bgewm p img {width: 100%; height: 100%;}
.bgewm span {font-size: .16rem; display: block; white-space: nowrap; text-align: center;}

.bgloading {position: absolute; top: 50%; left: 50%; transform: translate(-50%,-50%);}
.van-loading__text {color: #F63B75;}
...
.haibao .tip span {display: inline-block; width: 100%; white-space: nowrap; transform: scale(.8);}
...
.footer-title-modules:after{content: "";height: 1px;background-color: #909090;width: 3.63rem;position: absolute;left: 0;right: 0;margin: auto; z-index: 9;top: 0;bottom: 0}
.footer-title-modules p{position: absolute;left: 0;right: 0;margin: auto;width: 2.5rem;z-index: 99;line-height: .5rem; background-color: #F6F6F6;text-align: center;color: #909090;font-size: .32rem}
